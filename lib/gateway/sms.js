...
var addReport = function( report, monitor, timestamp, callback ) {
	...
	db.monitors.update({_id: report.monitor_id}, { $set: { status: report.status } }, { multi: false }, function (err, numReplaced) {
		//done
	})
}
			...
			db.monitors.insert({
				...
				addReport( report, monitor, timestamp, function( err, newReport ) {
					if ( err || !newReport )
					{
						...
						return res.status(200).send(newReport._id);
				} );
			})
		}
		else
